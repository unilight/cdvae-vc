...
class VAEUGAN(object):
    def __init__(self, arch):
        '''
        Variational Auto Encoder Unsupervised GAN (VAEUGAN)
        Arguments:
    ...
    def loss(self, data):
        ...
        sp_logit = self.sp_dis(x_sp)
        sp_sp_logit = self.sp_dis(x_sp_sp)
        dis_loss_sp = tf.reduce_mean(sp_logit - sp_sp_logit)

        loss = dict()
        loss['D_KL'] = kl_loss_sp
        loss['recon'] = recon_loss_sp                        
        loss['dis_sp2sp'] = dis_loss_sp
        
        with tf.name_scope('Summary'):

            tf.summary.scalar('KL-div-sp', kl_loss_sp)
            tf.summary.scalar('reconstruction-sp', recon_loss_sp)
            tf.summary.scalar('discriminate-sp2sp', dis_loss_sp)
